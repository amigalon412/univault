...
`Safex` is an ERC-4626 vault that takes USDG, issues shares, routes idle
...
|---|---|---|
| `Safex.t.sol` | no | Accounting, attacks, fuzz. The real test of our code. |
...
| `Safex.invariant.t.sol` | no | Solvency and supply properties over random call sequences. |
| `Safex.fork.t.sol` | yes | Wiring against live Robinhood Chain, plus assertions that pin the current state of the lending venue. |
...
export VAULT_NAME="Safex Balanced"
export VAULT_SYMBOL=sfxBALANCED
